...
public class AdjustArm2 extends SequentialCommandGroup {
	...
	public AdjustArm2 (double pivotAngle, double rackPosition, ArmMoveType moveType) {
		...
		case pickUp:
			addCommands(new movePivot(pivotAngle), new InstantCommand(() -> RobotContainer.s_ArmSubsystem.enableClaw(true)), 
			new moveRack(rackPosition), new InstantCommand(() ->RobotContainer.s_ArmSubsystem.enableVac(true)));
			break;
		case placeOnRobot:
			addCommands(new InstantCommand(() -> RobotContainer.s_ArmSubsystem.enableClaw(false)),
				new moveRack(rackPosition), new movePivot(pivotAngle),
			 new InstantCommand(() -> RobotContainer.s_ArmSubsystem.enableVac(false)));
			break;
		case PutInClaw:
			addCommands(new movePivot(pivotAngle), new moveRack(rackPosition),new InstantCommand(() -> RobotContainer.s_ArmSubsystem.enableClaw(true)),
					new InstantCommand(() -> RobotContainer.s_ArmSubsystem.enableVac(false)));
			break;
		case TakeFromClaw:
			addCommands(new InstantCommand(() -> RobotContainer.s_ArmSubsystem.enableVac(true)), 
					new InstantCommand(() -> RobotContainer.s_ArmSubsystem.enableClaw(false)),new movePivot(pivotAngle), new moveRack(rackPosition));
			break;
		}
	}
}